...
inline void _topk_fill_index_tile_(std::uint32_t dst_tile_index, std::uint32_t w)
{
    ...
    // units in either DEST mode; 4 per row group, +2 selects the odd columns).
    const std::uint32_t base = dst_tile_index * 64;
    for (std::uint32_t g = 0; g < 16; g++) // 4-row groups across the tile
    {
        TT_SFPSTORE(p_sfpu::LREG2, store_mode, ADDR_MOD_7, base + 4 * g);     // even columns
        TT_SFPSTORE(p_sfpu::LREG2, store_mode, ADDR_MOD_7, base + 4 * g + 2); // odd columns
