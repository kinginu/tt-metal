...
@pytest.mark.parametrize(
    ...
    [
        ...
        # TILE HEIGHT -> derived sharded out
        (
            (1, 1, 128, 64),
            (1, 1, 1, 2),
            lambda d: _height_shard_config((1, 1, 128, 64), d),
            _sharded_no_spec(ttnn.TensorMemoryLayout.HEIGHT_SHARDED),
            "tile_height_to_height_derived",
        ),
        ...
        # TILE BLOCK -> derived sharded out
        (
            (1, 1, 64, 64),
            (1, 1, 1, 2),
            lambda d: _block_shard_config((1, 1, 64, 64), d),
            _sharded_no_spec(ttnn.TensorMemoryLayout.BLOCK_SHARDED),
            "tile_block_to_block_derived",
        ),
    ],
)
...
# Dtype spot-checks (representative paths only; universal IO matrix covers bf16 exhaustively).
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param(
            ...
            _interleaved,
            ttnn.float32,
            id="f32_rm_interleaved",
        ),
    ],
)
def test_repeat_dtype_spot_checks(shape, repeat_shape, layout, in_mc_fn, dtype, device):
    run_repeat_test(
        shape,
        repeat_shape,
        device,
...
# RM interleaved → BLOCK/WIDTH sharded output without shard_spec.
@pytest.mark.parametrize(
    "shape, repeat_shape, memory_layout",
    [
        pytest.param((1, 1, 64, 128), (1, 1, 1, 2), ttnn.TensorMemoryLayout.BLOCK_SHARDED, id="block_out"),
        pytest.param((1, 1, 64, 128), (1, 1, 1, 2), ttnn.TensorMemoryLayout.WIDTH_SHARDED, id="width_out"),
        pytest.param((1, 4, 32, 64), (1, 2, 1, 1), ttnn.TensorMemoryLayout.BLOCK_SHARDED, id="C_repeat_block_out"),
    ],
)
def test_repeat_rm_interleaved_to_sharded_nospec(shape, repeat_shape, memory_layout, device):
    out_mc = ttnn.MemoryConfig(memory_layout, ttnn.BufferType.L1)
    run_repeat_test(
        shape,
        repeat_shape,
