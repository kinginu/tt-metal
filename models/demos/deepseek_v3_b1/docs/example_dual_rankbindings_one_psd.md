...
- **In-tree today (`tt-run` / mock / CPU):**
  ...
  - Fixtures under **`tests/tt_metal/distributed/config/`**, **`tests/tt_metal/tt_fabric/custom_mock_cluster_descriptors/`**; CI: **`MpiSubContext.SingleGalaxySplitContext`** (**`--rank-bindings-mapping`**) and **Generate Rank Bindings E2E** (**`-M`** + **`--mock-cluster-rank-binding`**). See **`ttnn/ttnn/distributed/README_ttrun.md`**.
...
| `tests/tt_metal/tt_fabric/custom_mock_cluster_descriptors/mock_galaxy_quad_2x4_four_rank_cluster_desc_mapping.yaml` | Mock cluster mapping → `bh_6u_cluster_desc.yaml` (ranks 0–3); used with **`--rank-bindings-mapping`** demo |
