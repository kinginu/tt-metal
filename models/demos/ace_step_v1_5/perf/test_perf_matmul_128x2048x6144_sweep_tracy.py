...
"""Program-config + memory-layout + fidelity sweep for a SINGLE matmul: M=128, K=2048, N=6144.

This is the 5 Hz LM **prefill MLP gate / up projection** (``gate_proj`` / ``up_proj`` — two per
layer, same shape).  Tracy reports it at HiFi4 / BF16 running ~115 us on 32 cores (8x4):

    128 x 2048 x 6144   HiFi4 BF16 x BF16 => BF16   115 us   32c   42.9% DRAM   28.1 TFLOPs (63.5%)
...
Swept axes:
  ...
  - grid (gx, gy), capped to the device grid.  (Nt=192 → core counts that divide 192.)
  - in0_block_w in {2, 4, 8}.
  - memory layout in0/out (l1 / dram / ws); weights stream from DRAM interleaved.

Note on width-sharded output: unlike QKV / o_proj / down (whose consumers want interleaved),
``gate``/``up`` feed the SwiGLU mul → ``down`` and CAN stay width-sharded without an inserted
``ShardedToInterleaved`` — so a ``ws``-out winner here may be usable in the model if the mul and
``down`` are wired to consume sharded.  The ``ws`` rows are still reported as matmul-only time.

Baseline (must-beat): ``(HiFi4, bf16)``, 1D ``l1/dram/l1``, grid 8x4 (32 cores), in0_block_w=8.
``_BASELINE_US`` left None uses that row's measured time; set it to 115.0 to gate against the
...
Run from the repository root **as plain pytest** (NOT under ``python -m tracy``)::
    ...
    TT_METAL_DEVICE_PROFILER=1 pytest \\
        models/demos/ace_step_v1_5/perf/test_perf_matmul_128x2048x6144_sweep_tracy.py::test_matmul_128x2048x6144_sweep \\
        -v -s
...
# (Perf report shows ~115 us for the HiFi4/bf16 32-core config.)
_BASELINE_US = None
...
# 1D grids whose core count divides Nt=192, fitting an 8x8 grid (safe on WH + BH).
...
# 2D grids: gy divides Mt=4 (gy in {2,4}), gx divides Nt=192.
_GRIDS_2D = [(8, 4), (8, 2), (4, 4), (4, 2), (2, 2)]
# Contrast grids for the exploratory in0=DRAM rows.
_GRIDS_CONTRAST = [(8, 4), (4, 2), (2, 2)]
# ws/ws rows: (grid, w) where kt_per_core = Kt/num_cores >= 2 and w | kt_per_core; cores | Nt=192.
...
def _make_cases():
    ...
    for fid in fids[:2]:
        # 1D, out L1 width-sharded (gate/up can chain sharded into the SwiGLU mul + down)
        for gx, gy in grids_1d:
            for w in (2, 4, 8):
...
def test_matmul_128x2048x6144_sweep(device):
