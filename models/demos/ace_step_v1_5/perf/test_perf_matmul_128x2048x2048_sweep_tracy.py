...
"""Program-config + memory-layout + fidelity sweep for a SINGLE matmul: M=128, K=2048, N=2048.

This is the 5 Hz LM **prefill attention output projection** (``o_proj`` / ``wo``).  Tracy reports
it at HiFi4 / BF16 running ~41 us on 32 cores (8x4):

    128 x 2048 x 2048   HiFi4 BF16 x BF16 => BF16   41 us   32c   40.2% DRAM   26.4 TFLOPs (59.6%)

Same sweep as the 128x2048x4096 QKV sibling — fixes the shape and sweeps the levers the
auto-tuner flags on these prefill linears: **math fidelity / weight dtype**, **in0_block_w (>=2)**,
and **out_subblock (h*w>=2 where the layout allows)** — plus program-config family, grid, and
memory layout, and reads off the fastest variant that still passes PCC vs an fp32 reference.
...
Swept axes:
  - fidelity/dtype : ``(HiFi4, bf16)`` is the current default (the baseline).  ``(HiFi2, bf16)``
    keeps bf16 weights but halves the fidelity phases; ``(HiFi2, bfp8)`` and ``(LoFi, bfp8)``
    additionally drop weights to ``bfloat8_b`` (halves weight DRAM BW).  Activations (in0) and
    output stay bf16 to match the model's prefill activation path.
...
Note on width-sharded output: ``o_proj``'s result feeds the residual add (an elementwise op that
wants interleaved input), so a ``ws`` output would force a ``ShardedToInterleaved`` in the real
model.  The ``ws`` rows are kept as exploration (matmul-only time); pin an ``l1``-out winner
...
to 41.0 to gate against the perf-report number directly.
...
Run from the repository root **as plain pytest** (NOT under ``python -m tracy``)::
    ...
    TT_METAL_DEVICE_PROFILER=1 pytest \\
        models/demos/ace_step_v1_5/perf/test_perf_matmul_128x2048x2048_sweep_tracy.py::test_matmul_128x2048x2048_sweep \\
        -v -s
...
# (Perf report shows ~41 us for the HiFi4/bf16 32-core config.)
_BASELINE_US = None
...
# ws/ws rows: (grid, w) where kt_per_core = Kt/num_cores >= 2 and w | kt_per_core; cores | Nt=64.
...
def test_matmul_128x2048x2048_sweep(device):
