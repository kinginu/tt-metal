...
"""Program-config + memory-layout + fidelity sweep for a SINGLE matmul: M=128, K=6144, N=2048.

This is the 5 Hz LM **prefill MLP down projection** (``down_proj`` / ``w2``).  Tracy reports it at
HiFi4 / BF16 running ~109 us on 32 cores (8x4):

    128 x 6144 x 2048   HiFi4 BF16 x BF16 => BF16   109 us   32c   45.2% DRAM   29.6 TFLOPs (67.0%)

Same sweep as the 128x2048x4096 QKV sibling — fixes the shape and sweeps **math fidelity / weight
dtype**, **in0_block_w (>=2)**, **out_subblock (h*w>=2 where the layout allows)**, plus
program-config family, grid, and memory layout, and reads off the fastest variant that still
passes PCC vs an fp32 reference.

This is the K-heavy MLP matmul (Kt=192) — in0_block_w divides Kt, so {2,4,8} are all legal and
larger blocks are worth a look on the K-reduction.

M=128 ⇒ Mt=4 — DS family DROPPED (needs M==1 tile, decode-only); 2D block-sharding ADDED
(gy may divide Mt=4).

Swept axes:
  - fidelity/dtype : ``(HiFi4, bf16)`` baseline → ``(HiFi2, bf16)`` → ``(HiFi2, bfp8)`` →
    ``(LoFi, bfp8)``.  Activations (in0) and output stay bf16.
  - family : "1D" (mcast_in0) | "2D" (block-sharded).
  - grid (gx, gy), capped to the device grid.  (Nt=64 → core counts that divide 64.)
  - in0_block_w in {2, 4, 8} (all divide Kt=192).
  - memory layout in0/out (l1 / dram / ws); weights stream from DRAM interleaved.

Note on width-sharded output: ``down_proj``'s result feeds the residual add (an elementwise op
that wants interleaved input), so a ``ws`` output would force a ``ShardedToInterleaved`` in the
real model.  The ``ws`` rows are kept as exploration (matmul-only time); pin an ``l1``-out winner
unless you also make the residual add consume sharded.

Baseline (must-beat): ``(HiFi4, bf16)``, 1D ``l1/dram/l1``, grid 8x4 (32 cores), in0_block_w=8.
``_BASELINE_US`` left None uses that row's measured time; set it to 109.0 to gate against the
perf-report number directly.

Run from the repository root **as plain pytest** (NOT under ``python -m tracy``)::

    TT_METAL_DEVICE_PROFILER=1 pytest \\
        models/demos/ace_step_v1_5/perf/test_perf_matmul_128x6144x2048_sweep_tracy.py::test_matmul_128x6144x2048_sweep \\
        -v -s
...
``profile_log_device.csv`` between configs.  Do **not** wrap this in ``python -m tracy -p -r``.
"""
...
# 1D grids whose core count divides Nt=64, fitting an 8x8 grid (safe on WH + BH).
_GRIDS_1D = [(8, 8), (8, 4), (8, 2), (4, 4), (4, 2), (2, 2)]
# Blackhole L1 (~1.5 MiB/worker): drop the 64-core row to stay clear of the CB budget.
_GRIDS_1D_BLACKHOLE = [(8, 4), (8, 2), (4, 4), (4, 2), (2, 2)]
# 2D grids: gy divides Mt=4 (gy in {2,4}), gx divides Nt=64.
_GRIDS_2D = [(8, 4), (8, 2), (4, 4), (4, 2), (2, 2)]
# Contrast grids for the exploratory in0=DRAM rows.
_GRIDS_CONTRAST = [(8, 4), (4, 2), (2, 2)]
# ws/ws rows: (grid, w) where w | kt_per_core = Kt/num_cores (Kt=192) and cores | Nt=64.
...
def _make_cases():
    ...
    for fid in fids[:2]:
        # 1D, out L1 width-sharded (matmul-only; real consumer (residual add) would pay s2i)
        for gx, gy in grids_1d:
            for w in (2, 4, 8):
...
def test_matmul_128x6144x2048_sweep(device):
