...
// Row count along dims before repeat axis.
uint64_t leading_row_volume(const ttnn::Shape& shape, int32_t repeat_dim) {
    const auto rank = static_cast<int32_t>(shape.rank());
    if (repeat_dim < 0 || repeat_dim >= rank) {
        return 1;
    }
    uint64_t v = 1;
    for (int32_t i = 0; i < repeat_dim; ++i) {
...
bool is_replication_locally_contained(
    ...
    // Higher-dim: per-core rows must divide group_size (whole replica-groups per shard).
    const uint64_t per_core_rows = input_shard_spec.shape[0];
    if (per_core_rows == 0) {
        return false;
    }
    const uint64_t lrv = leading_row_volume(input_padded_shape, repeat_dim);
    const uint64_t group_size = lrv * static_cast<uint64_t>(input_padded_shape[repeat_dim]);
...
std::optional<ShardSpec> generate_repeat_shard_spec(
    ...
    int32_t /*repeat_dim*/,
    uint32_t /*num_repeats*/) {
    // padded_out_shape is post-repeat; extra args are for API symmetry only.
