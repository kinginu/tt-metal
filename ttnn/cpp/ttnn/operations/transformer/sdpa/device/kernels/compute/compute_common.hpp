...
void correction_block(
    ...
    constexpr uint32_t dst_reg_0 = 0;  // prev_max in, exp_max_diff out
    constexpr uint32_t dst_reg_1 = 1;  // worker_max in, exp_max_diff_2 out
    constexpr uint32_t dst_reg_2 = 2;  // prev_sum in, cur_sum out
    constexpr uint32_t dst_reg_3 = 3;  // worker_sum in, cur_max out
    ...
    for (uint32_t i = 0; i < num_head_tiles; i++) {
        ...
        copy_tile(cb_prev_sum, i, dst_reg_2);
        copy_tile(cb_worker_sum, i, dst_reg_3);
        ...
        pack_tile(dst_reg_2, cb_cur_sum);
        pack_tile(dst_reg_3, cb_cur_max);
        tile_regs_release();
