...
// ===========================================================================
// H1b — line-multicast distances for a ring barrier / broadcast (all_gather)
// ===========================================================================
...
/**
 * @brief Forward/backward line-multicast range for a ring barrier.
 *
 * Thin wrapper over ccl::get_forward_backward_line_mcast_distance. The returned
 * {start_distance_in_hops, range_hops} feed the kernel's
 * ccl_routing_utils::line_multicast_route_info_t for a 1-D (LowLatencyPacketHeader)
 * multicast. The full 2-D per-direction (e/w/n/s) configuration, when needed, comes
 * from ccl::get_forward_backward_line_mcast_configuration — that stays op-side.
 */
inline DmMcastRoute ccl_dm_mcast_route(
    uint32_t ring_size, uint32_t ring_index, ttnn::ccl::Topology topology, bool is_forward) {
    auto [num_targets_forward, num_targets_backward] = ttnn::ccl::get_forward_backward_line_mcast_distance(
        ring_size, ring_index, topology, /*static_alternate=*/false);
    const uint32_t range = is_forward ? num_targets_forward : num_targets_backward;
    return {/*start_distance_in_hops=*/1, static_cast<uint16_t>(range)};
}
