...
UnifiedRoutedExpertFfnProgramFactory::cached_program_t UnifiedRoutedExpertFfnProgramFactory::create(
    ...
    // counts scratch: must hold up to MAX_GLOBAL_EXPERTS UINT32 entries so
    // indexing start[global_id] stays in bounds. ~4 KB/core — negligible.
    const uint32_t start_scratch_bytes =
        std::max<uint32_t>(static_cast<uint32_t>(start_buffer->aligned_page_size()), kScratchMinBytes);
