...
DispatchDeviceOperation::spec_return_value_t DispatchDeviceOperation::compute_output_specs(
    ...
    // use_fp8_dispatch (TILE path) packs bf16->fp8 into a UINT8-allocated buffer (legacy; content is
    // Fp8_e4m3). The row-major path moves token bytes verbatim, so the dispatched buffer inherits the
    // input dtype (BFLOAT16 -> BFLOAT16, FP8_E4M3 -> FP8_E4M3) with no UINT8 reinterpret.
    auto dispatch_buffer_dtype =
        operation_attributes.use_fp8_dispatch ? DataType::UINT8 : tensor_args.input_tensor.dtype();
