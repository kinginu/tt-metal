...
void DispatchDeviceOperation::validate_on_program_cache_miss(
    ...
    // Optional padding_config: per-device [local_real_tokens, pad_side], read on device
    // to bound the dispatch token loop. Must be ROW_MAJOR uint32/int32 with last dim 2.
